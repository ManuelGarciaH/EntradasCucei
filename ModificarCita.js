...
export default class ModificarCita extends Component {
  constructor(props) {
    ...
    let j=0;
    for (let i = 65; i <= 90; i++) {
        this.state.opcionesModulo.push({ key: i + j - 65, label: "Modulo " +  String.fromCharCode(i) });
    ...
    for (let i = 0; i < 30; i++) {
        ...
        this.state.opcionesFecha.push({ key: fechaFormateadaKey, label: fechaFormateada });
  ...
  llenarHoras=(fechaSeleccionada)=>{
    ...
    // Si es hoy o mañana, muestra las horas desde la actual hasta las 9:00 PM
        ...
        for (let hora = horaInicial; hora <= 21; hora++) {
            ...
            for (let minutos = hora === horaInicial ? this.redondearMinutos(minutoInicial) : 0; minutos < 60; minutos += 5) {
                const horaFormateada = format(new Date().setHours(hora, minutos), 'HH:mm');
                this.state.opcionesHora.push({ key: horaFormateada, label: horaFormateada });
  ...
  render() {
    const { nombreError, marcaAutoError, placasAutoError, diaError, horaEntradaError, puertaError,
      moduloDirigidoError } = this.state;
    ...
    const modificarCita=()=>{
      ...
      this.setState({ nombreError, placasAutoError, marcaAutoError, diaError, horaEntradaError, puertaError,
          moduloDirigidoError  });
      console.log(this.state.id)
      if(nombreError || placasAutoError || marcaAutoError || diaError || horaEntradaError
          || puertaError || moduloDirigidoError){
          ...
          xhttp.open("GET", "https://entradascucei.000webhostapp.com/modificarCita.php?nombre="+this.state.nombre+"&marcaAuto="+this.state.marcaAuto+"&placasAuto="+this.state.placasAuto+"&horaEntrada="+this.state.horaEntrada+"&dia="+this.state.dia+"&puerta="+this.state.puerta+"&moduloDirigido="+this.state.moduloDirigido+"&id="+this.state.id, true);
          ...
          <ScrollView style={styles.scrollContainer}>
            <View style={globalStyles.centrar}>
                ...
                <TextInput
                    ...
                    onChangeText={marcaAuto=>this.setState({marcaAuto})}
                ></TextInput>
                {marcaAutoError && <Text style={styles.errorMessage}>Campo requerido</Text>}
                ...
                <TextInput
                  defaultValue={this.props.route.params.response.placaAuto}
                    ...
                    onChangeText={placasAuto=>this.setState({placasAuto})}
                ></TextInput>
                {placasAutoError && <Text style={styles.errorMessage}>Campo requerido</Text>}
                ...
                <Text style={styles.textoAnterior}>Seleccion anterior: {this.props.route.params.response.diaEntrada}</Text>
                {diaError && <Text style={styles.errorMessage}>Campo requerido</Text>}
                ...
                <Text style={styles.textoAnterior}>Seleccion anterior: {this.props.route.params.response.horaEntrada}</Text>
                {horaEntradaError && <Text style={styles.errorMessage}>Campo requerido</Text>}
                ...
                <Text style={styles.textoAnterior}>Seleccion anterior: {this.props.route.params.response.puertaEntrada}</Text>
                {puertaError && <Text style={styles.errorMessage}>Campo requerido</Text>}
                ...
                <Text style={styles.textoAnterior}>Seleccion anterior: {this.props.route.params.response.moduloDirigido}</Text>
                {moduloDirigidoError && <Text style={styles.errorMessage}>Campo requerido</Text>}
